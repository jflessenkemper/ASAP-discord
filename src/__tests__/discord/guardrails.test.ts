...
describe('guardrails', () => {
  ...
  describe('classifyInput() with model classification', () => {
    beforeEach(() => {
      jest.resetModules();
      jest.clearAllMocks();
      process.env.GUARDRAILS_ENABLED = 'true';
    ...
    it('classifies safe input via the configured model', async () => {
      mockGenerateAnthropicText.mockResolvedValue('{"verdict": "safe", "confidence": 0.99, "reason": "normal question"}');
      const { classifyInput: ci } = await import('../../discord/guardrails');
      const result = await ci('What is the weather?', 'developer');
      expect(result.verdict).toBe('pass');
    });

    it('classifies injection with high confidence as block', async () => {
      mockGenerateAnthropicText.mockResolvedValue('{"verdict": "injection", "confidence": 0.85, "reason": "prompt injection"}');
      const { classifyInput: ci } = await import('../../discord/guardrails');
    ...
    it('classifies threat with low confidence → warn', async () => {
      mockGenerateAnthropicText.mockResolvedValue('{"verdict": "harmful", "confidence": 0.5, "reason": "possibly harmful"}');
      const { classifyInput: ci } = await import('../../discord/guardrails');
    ...
    it('returns pass when classify throws', async () => {
      ...
      mockGenerateAnthropicText.mockRejectedValue(new Error('API error'));
      const { classifyInput: ci } = await import('../../discord/guardrails');
      const result = await ci('Hello bot', 'developer');
      expect(result.verdict).toBe('pass');
      warnSpy.mockRestore();
    ...
    it('returns warn when JSON has no recognizable verdict', async () => {
      mockGenerateAnthropicText.mockResolvedValue('{"something": "else"}');
      const { classifyInput: ci } = await import('../../discord/guardrails');
      const result = await ci('Hello bot', 'developer');
      expect(result.verdict).toBe('warn');
    });
    ...
    it('returns pass when response is not JSON', async () => {
      mockGenerateAnthropicText.mockResolvedValue('not json at all');
      const { classifyInput: ci } = await import('../../discord/guardrails');
      const result = await ci('Hello bot', 'developer');
      expect(result.verdict).toBe('pass');
    });
    ...
    it('logs non-pass verdicts', async () => {
      mockGenerateAnthropicText.mockResolvedValue('{"verdict": "data_exfil", "confidence": 0.9, "reason": "data extraction"}');
      const { classifyInput: ci } = await import('../../discord/guardrails');
