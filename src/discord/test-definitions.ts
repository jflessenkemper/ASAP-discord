...
const RAW_AGENT_CAPABILITY_TESTS: AgentCapabilityTest[] = [
  ...
  // Cortana execution: tool-proof tests
  {
    ...
    prompt: 'Use read_file to read src/index.ts and summarize how the /api/health endpoint behaves in one line.',
    expectAny: [/health|status|timestamp|endpoint|route|ok/i],
  ...
  },
  {
    ...
    prompt: 'Use search_files to find the Express webhook endpoints in src/index.ts and reply with one file path.',
    expectAny: [/src\/index\.ts|webhook|express|route|endpoint/i],
  ...
  },
  {
    ...
    expectAny: [/table|schema|agent_memory|agent_activity_log|job_listings|job_profile|trace_spans/i],
  ...
  // Security auditor: code review
  {
    ...
    prompt: 'Use read_file to read src/index.ts and identify one security-sensitive endpoint or guard in one sentence.',
    expectAny: [/security|webhook|signature|secret|token|guard|unauthorized|concern/i],
    expectToolAudit: ['read_file'],
  },
  ...
  // API reviewer: route review
  {
    ...
    prompt: 'Use search_files to find the HTTP endpoints defined in src/index.ts and name one that has an input contract worth documenting more clearly.',
    expectAny: [/endpoint|route|health|metrics|agent-log|webhook|contract|document/i],
    expectToolAudit: ['search_files'],
  },
  ...
  // Performance: code analysis
  {
    ...
    prompt: 'Use read_file on src/discord/contextCache.ts and identify one performance-related optimization or measurement in one sentence.',
    expectAny: [/performance|cache|hit|miss|optimi|latency|metric|bottleneck/i],
    expectToolAudit: ['read_file'],
  },
  ...
  // Orchestration: specialist chain via Cortana
  {
    ...
    prompt: 'Without using any tools, write a short paragraph explaining why security-sensitive migrations should involve both QA and security specialists. Keep it to 3-4 sentences.',
    expectAny: [/qa|security|review|specialist|audit|migration/i],
    timeoutMs: 120_000,
    critical: false,
  },
  ...
  },
  {
    ...
    expectAll: [/err instanceof Error|agent_memory|agent_activity_log|secret|tsc|DDL/i, /test|lint|check|enforce|assert/i],
    expectToolAudit: ['read_file'],
    timeoutMs: 120_000,
  },
  ...
  },
  {
    id: 'developer',
    category: 'self-improvement',
    capability: 'migration-expectation-awareness',
    prompt: 'Use read_file on src/db/migrate.ts and explain which runtime tables are treated as required and why migration expectation checks matter before new migrations run.',
    expectAny: [/agent_memory|agent_activity_log|runtime|migration|drift|expected/i],
    expectToolAudit: ['read_file'],
    timeoutMs: 180_000,
  },
  {
  ...
  },
  {
    ...
    prompt: 'Use search_files to find any raw "INSERT INTO agent_memory" SQL outside of src/discord/memory.ts, src/discord/tools.ts, and src/discord/agents.ts. Report what you find and whether it matches the allowed persistence sites.',
  ...
  },
  {
    ...
    expectAny: [/errMsg|upsertMemory|appendMemoryRow|readMemoryRow|3.*times|extract|helper|enforce|review/i],
    expectToolAudit: ['read_file'],
    timeoutMs: 120_000,
  },
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    prompt: 'Use read_file to read src/discord/tools.ts. Describe the CircuitBreaker class, its 3 states, how TOOL_SERVICE_MAP groups tools into services, and what happens when failures accumulate.',
  ...
  },
  {
    ...
    prompt: 'Use search_files to find a function in src/discord/, src/utils/, or src/services/ that lacks test coverage in src/__tests__/. Propose a specific test case for it (describe the test, input, expected output). Do NOT write the test file — just describe what should be tested.',
  ...
  },
  {
    ...
    prompt: 'Use read_file to read src/index.ts and identify one user-facing error message that could be improved. Rewrite it in a friendlier tone.',
  ...
  },
  {
    ...
    prompt: 'Read .github/cortana-system-prompt.md and describe your communication style in 2-3 sentences. What do "measured" and "dry" mean for how you talk?',
    expectAny: [/measured|dry|sardonic|short|concise|direct|warm|cortana/i],
    expectToolAudit: ['read_file'],
    timeoutMs: 120_000,
  },
  {
    ...
    prompt: 'Read .github/cortana-agent-roster.md and list 3 specialist agents from the pantheon with their role.',
    expectAny: [/argus|aphrodite|athena|iris|mnemosyne|hermes|hephaestus|calliope|themis|artemis|prometheus/i],
    expectToolAudit: ['read_file'],
    timeoutMs: 120_000,
  },
];
