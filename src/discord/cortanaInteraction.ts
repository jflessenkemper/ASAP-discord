...
export function buildGroupchatDecisionAttention(targetChannelId: string, groupchatId: string, configuredUserId?: string | null): string {
  ...
  return `<@${userId}> Cortana needs a decision from you here.`;
}
...
export function buildVoiceSingleSpeakerNotice(activeSpeakerName?: string): string {
  ...
  if (!active) {
    return '🟡 One speaker at a time please. Cortana can only respond to one voice turn right now.';
  }
  return `🟡 One speaker at a time please. Cortana is currently handling ${active} and will listen again when that turn is finished.`;
}

export function buildGroupchatSingleUserNotice(activeUserName?: string): string {
  const active = String(activeUserName || '').trim();
  if (!active) {
    return '🟡 One person at a time please. Cortana is already handling another request in groupchat.';
  }
  return `🟡 One person at a time please. Cortana is currently handling ${active}'s request in groupchat.`;
}
