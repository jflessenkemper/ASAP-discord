...
export const cortanaConfig = {
  stallNoticeCooldownMs: () =>
    envIntFirst(['CORTANA_STALL_NOTICE_COOLDOWN_MS', 'RILEY_STALL_NOTICE_COOLDOWN_MS'], 120_000),
  noResponseTimeoutMs: () =>
    envIntFirst(['CORTANA_NO_RESPONSE_TIMEOUT_MS', 'RILEY_NO_RESPONSE_TIMEOUT_MS'], 180_000),
  ...
  tokenOverrunAllowance: () =>
    envIntFirst(['CORTANA_TOKEN_OVERRUN_ALLOWANCE', 'RILEY_TOKEN_OVERRUN_ALLOWANCE'], 2_000_000),
  maxContinuationCycles: () =>
    envIntFirst(['CORTANA_MAX_CONTINUATION_CYCLES', 'RILEY_MAX_CONTINUATION_CYCLES'], 3),
  allowImplicitInfraActions: () =>
    envBoolFirst(['CORTANA_ALLOW_IMPLICIT_INFRA_ACTIONS', 'RILEY_ALLOW_IMPLICIT_INFRA_ACTIONS']),
  autoApproveBudgetIncrementUsd: () => {
    const raw = envFirst(['CORTANA_AUTO_APPROVE_BUDGET_INCREMENT_USD', 'RILEY_AUTO_APPROVE_BUDGET_INCREMENT_USD'], '5');
    const n = parseFloat(raw);
    return Number.isFinite(n) ? n : 5;
  },
  autoApproveBudgetMaxPasses: () =>
    envIntFirst(['CORTANA_AUTO_APPROVE_BUDGET_MAX_PASSES', 'RILEY_AUTO_APPROVE_BUDGET_MAX_PASSES'], 4),
};

/** ElevenLabs/telephony voice naming. */
export const voiceConfig = {
  telephonyVoiceName: () =>
    envFirst(['TELEPHONY_CORTANA_VOICE_NAME', 'TELEPHONY_RILEY_VOICE_NAME'], 'RileyEL'),
  cortanaMaxTokens: (lowLatency: boolean) =>
    envIntFirst(
      ['VOICE_MAX_TOKENS_CORTANA', 'VOICE_MAX_TOKENS_RILEY'],
      lowLatency ? 120 : 220,
    ),
};
